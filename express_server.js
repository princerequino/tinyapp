...
app.get("/urls/:shortURL", (req, res) => {
  ...
  if (userDatabase[userShortUrl] === undefined) {
    res
      .status(404)
      .send(
        `<html><body><p><b> Please <a href='/login'>log in</a> with a valid account. If not registered, click <a href='/register'>register</a> here!<b></body></html>`
      );
  } else {
    const templateVars = {
...
app.get("/u/:shortURL", (req, res) => {
  if (urlDatabase[req.params.shortURL] === undefined) {
    res
      .status(404)
      .send(
        "You have requested a non-existent shortURL!  </br><html><body><a href=/>HOME</a></body></html>"
      );
  } else {
...
// "*"" --> wildcard
//executes if the link being accessed is NOT FOUND in ANY of my declared routes.
app.use("*", (req, res) => {
  res
    .status(404)
    .send(
      "You have entered an invalid link!  </br><html><body><a href=/>HOME</a></body></html>"
    );
});
